...
**Текущая воспроизводимая локальная проверка:** Linux/Node 24: `npm run check`, `node scripts/check-syntax.mjs` (232 JS/MJS/CJS-файла), `node --test` — 852 total / 845 passed / 0 failed / 7 skipped, `npm run test:store` — passed; headless Chromium/WebGL2 SwiftShader smoke проверил streamed node/attribute draw. Это текущее локальное рабочее дерево, а не clean checkout. Hosted GitHub Actions run `36181982890` проверял предыдущий source commit `76173ad`; текущие локальные изменения v2/WebGL туда ещё не отправлены и не прошли hosted CI. Windows CI — не packaged installer/ASAR и не GPU UI acceptance. Skips требуют внешние fixtures или optional decoder. Packaged Windows/ASAR, целевой GPU/VRAM, независимые CAD/BIM/GIS readers, полный CRS/datum, COPC, универсальный IFC/DWG и 100 GB benchmark не сертифицированы. См. `QA-RETEST-stage3.md` и `QA-RETEST-stage4.md`.
...
- **Инкремент review 1.1.17 / Stage 3–4 (checkpoint):** format I/O, PTX multi-scan writer, E57 scan/pose round-trip, viewer scan-order protection and bounded PLY/LAS/PCD ingest have targeted unit/UI coverage. Последний локальный прогон: 852 tests (845 passed / 0 failed / 7 fixture-or-decoder skips), `npm run check`, syntax-check 232 files и `npm run test:store` all pass; streamed v2 intensity/classification renderer получил async fake-GL regression, stale-response guard/backoff tests и headless WebGL2 smoke. Exact user-fixture files, identifiers, performance details and local harness artifacts are excluded from Git. Hosted CI for this exact local change set ещё не запускался. Stage 3/4 are still partial and not accepted; see the current QA re-test notes and outstanding gates.
