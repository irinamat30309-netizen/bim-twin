...
- До приёмки остаются расширение chunked ingest на другие форматы, атрибуты и stream-aware инструменты/export, persistent/resumable stores, pressure/cold-repeat benches, multi-size целевые GPU/VRAM проверки и clean-machine Windows install/launch. Build/ASAR content verification уже пройдены в private run #9. Конкретные тесты и ограничения приведены в `QA-RETEST-stage4.md`.
