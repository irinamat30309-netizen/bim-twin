...
**Последняя воспроизводимая проверка:** review-ветка `agent/windows-package-qa`, GPU-selection code commit `4c6cddeede0ea05387afdb028341de59f5b46a60`, плюс Linux/Node 24 retest: `npm test` — 883 total / 880 passed / 0 failed / 3 skipped без пользовательских файлов; с ранее предоставленным локальным OBJ/STL/PLY набором — 883 / 882 / 0 / 1 (только physical GPU skip). Focused Stage 2/3/4/5–7 — 31/41/81/103 passed; LAZ suite — 5/5; `npm run check`, JS syntax (241 файлов), Python syntax (28 файлов), PLY syntax и `npm run test:store` — passed. GPU-код явно запрашивает high-performance WebGL; self-hosted smoke рисует 1 млн точек 10 секунд и требует NVIDIA adapter, но физически на RTX 5070 после этого изменения ещё не запущен. GitHub Actions на code commit: **4/4 passed** (`test`, `windows-test`, два `windows-package`). Подробный статус — в `QA-RETEST-stage0-7-current.md`.
