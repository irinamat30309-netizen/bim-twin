...
| Portable Linux `node --test --test-reporter=tap`, без пользовательских fixtures | **883 total / 880 passed / 0 failed / 3 skipped**: две проверки требуют внешние OBJ/STL/large-PLY fixtures и self-hosted GPU smoke, который на Linux runner не запускается |
| Полный набор с ранее предоставленным локальным OBJ/STL/PLY mount (Linux) | **883 total / 882 passed / 0 failed / 1 skipped**: только self-hosted Windows GPU smoke |
| Private Windows hardware-QA run #9, public commit `0746bf3` | **883 total / 880 passed / 0 failed / 3 skipped**; RTX smoke прошёл, пропущены две недоступные user fixtures и Linux-only DAC denial test |
...
| Private Windows hardware-QA run #9 на public commit `0746bf3` | **Passed, 5:36**; `[BIMTWIN_GPU_WEBGL]` показал `ANGLE (NVIDIA GeForce RTX 5070, Direct3D11)`, WebGL2, 1 000 000 точек за 10 013 ms/1 606 кадров, 533 200 изменённых пикселей и `nvidiaAdapterDetected=true`; smoke passed за 11 581 ms. Полный suite: 883 / 880 passed / 0 failed / 3 skipped (2 user fixtures + Linux-only DAC). Поле `activeAdapters` было пустым, поэтому вывод основан на фактическом NVIDIA renderer и детекторе smoke, а не на этом поле.
...
5. Основные WebGL viewer contexts запрашивают `powerPreference: high-performance`; hardware smoke включает Electron high-performance GPU flags, непрерывно рисует 1 000 000 точек в течение 10 секунд и требует NVIDIA adapter вместо software renderer. Private run #9 на RTX 5070 прошёл; это подтверждает rendering, но не CUDA compute и не крупный VRAM stress benchmark.

## Пропуски полного набора

- В portable Linux full suite без `BIM_TWIN_USER_FIXTURES` пропущены две проверки user-supplied OBJ/STL/large-PLY и self-hosted NVIDIA smoke; на private Windows run #9 этот smoke выполнился на RTX 5070 и прошёл.
- В run #9 две внешние dataset-проверки остались пропущены, поскольку файлы не были подключены; третий skip — Linux-only DAC denial test. Ранее предоставленный OBJ/STL/PLY набор отдельно проходил локально 12/12; файлы не копировались и не коммитились.
...
- **Stage 4:** target RTX 5070 WebGL render smoke пройден; остаются full-cloud/out-of-core операции для остальных форматов, persistent/resumable stores, pressure/cold-repeat tests и крупные VRAM/performance benchmarks.
