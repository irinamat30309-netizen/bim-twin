...
| Проверка | Результат |
|---|---|
| Повторный `node --test` на текущем локальном source tree (без локальных fixture-файлов и optional LAZ decoder) | 852 теста: 845 passed, 0 failed, 7 skipped; пропущены только сценарии, которым недоступны нужные fixtures/decoder |
...
| Hosted GitHub Actions CI | Linux `test` and Windows `windows-test` passed for commit `76173ad`; packaged Windows/ASAR remains untested |
...
**Вывод:** Stage 4 расширяет bounded-working-set ingest и disk octree для ASCII/binary point-cloud PLY с поддерживаемыми scalar properties, uncompressed LAS formats 0–10 и PCD ASCII/interleaved-binary/LZF; LZF preview использует ограниченный disk scratch и point budget. Текущий локальный regression suite прошёл 845/852 теста, 0 failures, 7 fixture/decoder skips; renderer integration и headless WebGL smoke прошли. Это не эквивалент Windows UI/GPU приёмки. Этап остаётся **частичным, не принят и не завершён**: другие форматы/mesh memory-bound, произвольные/extra dimensions и full-stream инструменты не реализованы, нет resumable stores; hosted Windows CI относится к предыдущему commit, текущие локальные изменения в нём не проверялись. По-прежнему нужны packaged/ASAR и GPU/VRAM проверки, повторяемые benchmarks и CAD/BIM/GIS round-trip.
