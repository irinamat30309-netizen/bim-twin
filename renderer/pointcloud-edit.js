...
(function () {
  ...
  // занятых вокселей с ребром voxel ≈ r), и остаются ��ишь кластеры у переднего края:
  ...
  function cleanAuto(cloud, opts){
    ...
    for (var p = 0; p < maxPasses; p++){
      ...
      // SOR (k-NN) — самый дорогой шаг. На больших облаках он морозит UI-поток, поэто��������������������у пропускаем его выше порога (нативный Open3D-движок обрабатывает такие облака отдельно).
  ...
  function protectFloorLocal(cloud, indices, opts){
    ...
    // больше стен/пола/потолка ловим ��а счёт меньшего minFrac и ��ольшего maxPlanes.
  ...
  // =========================================================================
  // v1046 — Phase 1-3: расширенный конвей��р обработки (по образцу CloudCompare/Open3D).
  ...
  // Собственные зна��ения/векторы симметричной матрицы NxN методом Якоби (N=3 или 4).
