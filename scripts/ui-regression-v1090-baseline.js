...
(async function () {
  ...
  await check('Save measurement survives exiting mode',()=>{const before=Number(($('mmList').textContent.match(/\((\d+)\)/)||[])[1]||0);$('mmSave').click();const saved=$('mmList').textContent;eq(Number((saved.match(/\((\d+)\)/)||[])[1]),before+1,'not saved');$('mmExit').click();eq(v.measuring,false,'mode remains');eq(visible($('measureBar')),false,'bar remains');eq($('mmList').textContent,saved,'saved result lost');});
  ...
  await check('Finish figure differs from exit',()=>{$('btnMeasure').click();$('mmPolyline').click();v._measureClick([0,0,0]);v._measureClick([1,0,0]);$('mmFinish').click();ok(v.measuring,'finish must not exit');$('mmExit').click();eq(v.measuring,false,'close button did not exit');});
