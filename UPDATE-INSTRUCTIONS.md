...
**Важно:** изменения уже опубликованы в review-ветке `agent/windows-package-qa` и PR #2; повторно применять overlay или делать push не нужно. На QA commit `85e05cb5a3fa37fb259a056cf4016f752b6bedb7` GitHub Actions завершились **4/4 passed**, включая hosted Windows test/package checks. Это не подтверждает запуск установленного приложения на чистой машине, физическую GPU/VRAM-нагрузку, внешний CAD/GIS/BIM обмен или независимую пользовательскую приёмку. Для hardware acceptance именно текущего head повторно запустите приватный Windows workflow с `source_ref=agent/windows-package-qa`; приватный runner не следует подключать к публичному репозиторию. Не отправляйте PAT, runner registration token или другие секреты в чат.
