...
**Важно:** изменения опубликованы в review-ветке `agent/windows-package-qa` и PR #2. Private Windows hardware-QA run #9 уже проверил public commit `0746bf3`: WebGL renderer вывел NVIDIA GeForce RTX 5070/Direct3D11, WebGL2, 1 млн точек/10 секунд/1 606 кадров; `nvidiaAdapterDetected=true`, smoke прошёл. NSIS/ASAR build и проверка содержимого ASAR также прошли. Это подтверждает именно rendering и упаковку, но не CUDA-ускорение parsing/geometry, не multi-size VRAM stress и не чистую установку приложения. Обычные parsing/build/geometry задачи остаются CPU-bound; GPU используется при WebGL-рендеринге и может простаивать между кадрами. Приватный runner не подключать к публичному репозиторию. Не отправляй PAT, runner registration token или другие секреты в чат.
