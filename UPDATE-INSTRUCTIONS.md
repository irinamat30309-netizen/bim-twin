...
**Важно:** изменения уже опубликованы в review-ветке `agent/windows-package-qa` и PR #2; повторно применять overlay или делать push не нужно. GPU-selection code commit `4c6cddeede0ea05387afdb028341de59f5b46a60` прошёл **4/4 hosted Windows checks**. WebGL viewers теперь запрашивают high-performance adapter; self-hosted smoke требует NVIDIA и создаёт 10-секундную нагрузку на 1 млн точек. Публичный hosted CI не может подтвердить RTX 5070, а скриншот приватного run #8 не показывает source_ref или renderer. Чтобы проверить твою карту, повторно запусти приватный workflow с `source_ref=agent/windows-package-qa` и пришли строку `[BIMTWIN_GPU_WEBGL]`; приватный runner не следует подключать к публичному репозиторию. Обычные parsing/build/geometry regression задачи остаются CPU-bound; GPU используется при WebGL-рендеринге, а не постоянно. Не отправляй PAT, runner registration token или другие секреты в чат.
